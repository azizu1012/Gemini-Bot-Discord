...
async def cleanup_local_files():
    """
    Dọn dẹp file trong thư mục FILE_STORAGE_PATH nếu dung lượng trống dưới ngưỡng.
    Ưu tiên xóa các file cũ nhất (dựa trên thời gian truy cập cuối).
    """
    try:
        current_free_space = get_disk_free_space_mb(FILE_STORAGE_PATH)
        
        if current_free_space > MIN_FREE_SPACE_MB:
            logger.info(f"Dung lượng trống: {current_free_space} MB. Không cần dọn dẹp file local.")
            return

        logger.warning(f"Dung lượng trống còn {current_free_space} MB (dưới ngưỡng {MIN_FREE_SPACE_MB} MB). Bắt đầu dọn dẹp file local...")
        
        files_to_delete = []
        
        # Lấy danh sách file và thời gian truy cập gần nhất (access time)
        for root, _, files in os.walk(FILE_STORAGE_PATH):
            for name in files:
        ...
        for path, _ in files_to_delete:
            try:
                ...
                current_free_space = get_disk_free_space_mb(FILE_STORAGE_PATH)
                if current_free_space > MIN_FREE_SPACE_MB + 50: # Thêm buffer 50MB
