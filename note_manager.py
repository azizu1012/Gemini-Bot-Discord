...
async def save_note_to_db(user_id: str, content: str, source: str) -> str:
    ...
    try:
        ...
        success = await db.add_user_note_db(user_id, note_id, content, metadata)
...
async def save_file_note_to_db(user_id: str, content: str, filename: str, source: str = "file_upload") -> bool:
    ...
    try:
        ...
        existing_note = await db.get_file_note_by_filename_db(user_id, filename)
        ...
        if existing_note:
            ...
            success = await db.update_user_note_db(existing_note['note_id'], content, metadata)
            if success:
...
async def retrieve_notes_from_db(user_id: str, query: str) -> str:
    ...
    try:
        ...
        notes: List[Dict[str, Any]] = await db.get_user_notes_db(user_id, search_query=query)
