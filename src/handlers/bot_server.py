...
def normalize_city_name(city_query):
    ...
    for k, v in config.CITY_NAME_MAP.items():
        if k in city_key:
...
# --- TẢI BIẾN MÔI TRƯỜNG ---
load_dotenv()
config.TOKEN = os.getenv('DISCORD_config.TOKEN')
...
config.MODEL_NAME = os.getenv('config.MODEL_NAME')
config.ADMIN_ID = os.getenv('config.ADMIN_ID', '')
config.HABE_USER_ID = os.getenv('config.HABE_USER_ID', '')
config.MIRA_USER_ID = os.getenv('config.MIRA_USER_ID', '')
config.ADO_FAT_USER_ID = os.getenv('config.ADO_FAT_USER_ID', '')
config.MUC_RIM_USER_ID = os.getenv('config.MUC_RIM_USER_ID', '')
config.SUC_VIEN_USER_ID = os.getenv('config.SUC_VIEN_USER_ID', '')
config.CHUI_USER_ID = os.getenv('config.CHUI_USER_ID', '')

# API Keys cho Search (từ .env)
config.SERPAPI_API_KEY = os.getenv('config.SERPAPI_API_KEY')
config.TAVILY_API_KEY = os.getenv('config.TAVILY_API_KEY')
config.EXA_API_KEY = os.getenv('config.EXA_API_KEY')
config.GOOGLE_CSE_ID = os.getenv('config.GOOGLE_CSE_ID')  # Đã có
config.GOOGLE_CSE_API_KEY = os.getenv('config.GOOGLE_CSE_API_KEY')  # Đã có
config.GOOGLE_CSE_ID_1 = os.getenv("config.GOOGLE_CSE_ID_1")
config.GOOGLE_CSE_API_KEY_1 = os.getenv("config.GOOGLE_CSE_API_KEY_1")
config.GOOGLE_CSE_ID_2 = os.getenv("config.GOOGLE_CSE_ID_2")
config.GOOGLE_CSE_API_KEY_2 = os.getenv("config.GOOGLE_CSE_API_KEY_2")
...
config.WEATHER_CACHE_PATH = os.path.join(os.path.dirname(__file__), 'weather_cache.json')
weather_lock = asyncio.Lock()  # Lock cho cache

# Lấy key & city từ .env
config.WEATHER_API_KEY = os.getenv('config.WEATHER_API_KEY')
config.CITY = os.getenv('config.CITY')

# --- ĐƯỜNG DẪN FILE (CẬP NHẬT) ---
config.DB_PATH = os.path.join(os.path.dirname(__file__), 'chat_history.db')
# (Thay đổi) Dùng 1 file backup cố định, không spam file theo ngày
config.DB_BACKUP_PATH = os.path.join(os.path.dirname(__file__),
                              'chat_history_backup.db')
config.NOTE_PATH = os.path.join(os.path.dirname(__file__), 'notes.txt')
# (Mới) File JSON cho bộ nhớ ngắn hạn
config.MEMORY_PATH = os.path.join(os.path.dirname(__file__), 'short_term_memory.json')
...
if GEMINI_API_KEY_PROD:
    config.GEMINI_API_KEYS.append(GEMINI_API_KEY_PROD)
if GEMINI_API_KEY_TEST:
    config.GEMINI_API_KEYS.append(GEMINI_API_KEY_TEST)
if GEMINI_API_KEY_BACKUP:
    config.GEMINI_API_KEYS.append(GEMINI_API_KEY_BACKUP)
if GEMINI_API_KEY_EXTRA1:
    config.GEMINI_API_KEYS.append(GEMINI_API_KEY_EXTRA1)
if GEMINI_API_KEY_EXTRA2:
    config.GEMINI_API_KEYS.append(GEMINI_API_KEY_EXTRA2)

if not config.GEMINI_API_KEYS:
    logger.error("Không tìm thấy Gemini API keys! Bot sẽ không thể hoạt động.")
else:
    logger.info(
        f"Đã thiết lập {len(config.GEMINI_API_KEYS)} Gemini API keys cho Failover.")
...
config.SPAM_THRESHOLD = 3
config.SPAM_WINDOW = 30
    ...
    if os.path.exists(config.DB_PATH):
        try:
            ...
            try:
                ...
                shutil.copy2(config.DB_PATH, config.DB_BACKUP_PATH)
                logger.info(f"DB backed up to {config.DB_BACKUP_PATH}")
            finally:
                conn.close()
    ...
    if not os.path.exists(config.MEMORY_PATH):
        try:
            with open(config.MEMORY_PATH, 'w', encoding='utf-8') as f:
                json.dump({}, f)
            logger.info(f"Created new short term memory file: {config.MEMORY_PATH}")
        except Exception as e:
...
async def get_weather(city_query=None):
    ...
    async with weather_lock:
        ...
        city_env = config.CITY or "Ho Chi Minh City"
        city_query = city_query or city_env
        city_en, city_vi = normalize_city_name(city_query)

        # Tạo cache riêng cho từng thành phố
        cache_path = config.WEATHER_CACHE_PATH.replace(".json", f"_{city_en.replace(' ', '_').lower()}.json")
        ...
        try:
            url = f"http://api.weatherapi.com/v1/forecast.json?key={config.WEATHER_API_KEY}&q={city_en}&days=7&aqi=no&alerts=no"
...
async def read_note():  # Thay def thành async def
    try:
        if not os.path.exists(config.NOTE_PATH):
            return "Chưa có note nào bro! Ghi note đi nha! 😎"
        async with aiofiles.open(config.NOTE_PATH, 'r', encoding='utf-8') as f:
...
def is_admin():
    async def predicate(interaction: discord.Interaction) -> bool:
        return str(interaction.user.id) == config.ADMIN_ID
    ...
    for q in enriched_queries:
        async with SEARCH_LOCK:
            ...
            cse0_task = asyncio.create_task(
                _search_cse(log_q, config.GOOGLE_CSE_ID, config.GOOGLE_CSE_API_KEY, 0, start_idx=1)
            )
            cse1_task = asyncio.create_task(
                _search_cse(log_q, config.GOOGLE_CSE_ID_1, config.GOOGLE_CSE_API_KEY_1, 1, start_idx=4)
            )
            cse2_task = asyncio.create_task(
                _search_cse(log_q, config.GOOGLE_CSE_ID_2, config.GOOGLE_CSE_API_KEY_2, 2, start_idx=7)
            )
...
async def _run_fallback_search(query):
    ...
    for i in range(3):
        ...
        try:
            if api_name == "SerpAPI" and config.SERPAPI_API_KEY:
                result = await _search_serpapi(query)
            elif api_name == "Tavily" and config.TAVILY_API_KEY:
                result = await _search_tavily(query)
            elif api_name == "Exa" and config.EXA_API_KEY:
...
async def _search_exa(query):
    ...
    if not config.EXA_API_KEY: return ""
    
    exa = exa_py.Exa(api_key=config.EXA_API_KEY)
    params = {
        "query": query, # Dùng query TỪ GEMINI
        "num_results": 3, 
        "use_autoprompt": True, 
...
async def on_message(message):
    ...
    if is_admin:
        ...
        for mid in mentioned_ids:
            ...
            if mid == config.ADMIN_ID and is_negative_comment(query):
...
# --- (SỬA ĐỔI) LẤY GIỜ VÀ ĐỊNH DẠNG ---
    ...
    try:
        start = datetime.now()
        reply = await run_gemini_api(messages, config.MODEL_NAME, user_id, temperature=0.7, max_tokens=2000)
