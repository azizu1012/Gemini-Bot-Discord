...
function Start-KafkaIfNeeded {
    ...
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $kafkaPidFile), (Split-Path -Parent $kafkaLogFile) | Out-Null
    $env:JAVA_HOME = $JavaDir
    $kafkaProc = Start-Process -FilePath $javaExe -ArgumentList @('-cp', $kafkaLibs, 'kafka.Kafka', $kafkaConfig) -RedirectStandardOutput $kafkaLogFile -RedirectStandardError $kafkaErrLogFile -PassThru -WindowStyle Hidden
