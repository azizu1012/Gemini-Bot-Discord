...
class Config:
    ...
    def _load_gemini_api_keys(self) -> None:
        ...
        if not self.GEMINI_API_KEYS:
            print("WARNING: Không tìm thấy Gemini API keys! Bot sẽ không thể hoạt động.")
        else:
            print(f"INFO: Đã thiết lập {len(self.GEMINI_API_KEYS)} Gemini API keys cho Proactive Rate Limiting.")
    ...
    def _load_proxy_config(self) -> None:
        ...
        if os.path.exists(config_ini_path):
            try:
                ...
                print(f"WARNING: Lỗi đọc config.ini: {e}. Sử dụng .env hoặc default.")
    ...
    def toggle_proxy(self, enabled: bool) -> bool:
        ...
        try:
            ...
            print(f"ERROR: Không thể cập nhật proxy config: {e}")
            return False
